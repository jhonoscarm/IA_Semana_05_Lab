...
public class Yogurt extends JFrame implements ActionListener {
	...
	protected void actionPerformedBtnProcesar(ActionEvent arg0) {
		...
		if(marca == "Laive"){pre=5.4;}
		else if(marca == "Gloria"){pre=5.7;}
		else if(marca == "Pura Vida"){pre=4.8;}
		else{pre=5.3;}
		
		impC = pre * cant;
		
		//Calculando importe de descuento
		if(cant<3){impD=impC*0;}
		else if(cant>=3&&cant<6){impD=impC*0.025;}
		else if(cant>=6&&cant<12){impD=impC*0.05;}
		else{impD=impC*0.075;}
		
		impP = impC - impD;
		
		//Calculando caramelos
		if(marca=="Laive"){cara = 2;}
		else if(marca=="Gloria"){cara = 3;}
		else if(marca=="Pura Vida"){cara = 4;}
		else{cara = cant * 2;}
		
		txtS.setText("	SISTEMA DE PROGRAMA DE YOGURT \n\n");
		imprimir("Importe compra    : S/. " + decimalFormat(impC));
		imprimir("Importe descuento : S/. " + decimalFormat(impD));
		imprimir("Importe pagar     : S/. " + decimalFormat(impP));
